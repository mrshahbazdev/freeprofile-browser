...
A minimal CEF (Chromium Embedded Framework) browser with per-profile isolation, proxy, and user-agent support. This is the starting scaffold for a GoLogin-style anti-detect browser.

## Features

- Per-profile isolated cache / cookies / storage (each process uses its own `cache_path`).
- Proxy support (`http://user:pass@host:port` or `socks5://...`).
- Custom user-agent per profile.
- Launch any starting URL.
...
Linux:
```bash
./build/Release/freeprofile-browser --url=https://example.com
```

With a profile, proxy, and custom UA:
```bash
...
Windows:
```cmd
Release\freeprofile-browser.exe --url=https://example.com
```
...
- This is an **MVP scaffold**. A production anti-detect browser needs:
